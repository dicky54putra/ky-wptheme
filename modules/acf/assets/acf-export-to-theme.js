(function ($) {
  ...
  fn.exportAcfToTheme = function exportAcfToTheme(e) {
    ...
    $.ajax({
      url: objKyAcf.ajaxURL,
      type: "POST",
      dataType: "json",
      data: {
        action: objKyAcf.action,
        nonce: objKyAcf.nonce,
  ...
  $(window).load(function () {
    ...
    if ($checkboxWrapper.length) {
      var $exportBtn = $(
        '<span class="button button-link-delete" id="acf-export-theme">Export as php file into theme</span><span class="loader"></span>'
      );

      $exportBtn.on("click", fn.exportAcfToTheme);

      $exportBtn.appendTo(
        $checkboxWrapper.parents("#acf-admin-tool-export").find(".acf-submit")
      );
    }
  });
